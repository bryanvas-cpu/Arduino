...
// --- Set motor RPMs (positive=forward, negative=reverse, 0=stop) ---
void motor_driver_set_omega(float omega1, float omega2) {
    ...
    // --- Motor2 direction ---
    if (omega2 < 0) digitalWrite(DIR_PIN2, HIGH); // oppposite mounted stepper
    ...
    float lin_vel = r * (omega_motor1 + omega_motor2) / 2.0;
    float ang_vel = r * (omega_motor2 - omega_motor1) / L;
