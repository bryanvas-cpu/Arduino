...
JOY_CMD processGamepad(ControllerPtr ctl) {
    ...
    static int body_height_cmd = 200;  // persistent between calls
    ...
    // Clamp body height between 150 and 300
    if (body_height_cmd > 300) body_height_cmd = 300;
    if (body_height_cmd < 150) body_height_cmd = 150;

    // Return command
    if (on_off_state == -1) {
        return JOY_CMD{on_off_state, body_height_cmd, 0.0, 0.0};
    } else {
        return JOY_CMD{on_off_state, body_height_cmd, ctl->axisRX(), ctl->axisY()};
    }
}
...
JOY_CMD processControllers() {
    ...
    return JOY_CMD{on_off_state, 0.0, 0.0}; // fallback if no data
}
...
JOY_CMD UpdateJoyloop() {
    ...
    if (dataUpdated) {
        ...
        return JOY_CMD{on_off_state, 0.0, 0.0};
    }
}
